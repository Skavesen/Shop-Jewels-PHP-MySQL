<?php
	...
	if($last != 1){
		...
		if ($pagenum > 1) {
			$First = $pagenum == 1;
			$previous = $pagenum - 1;
			$paginationCtrls .= '<a href="'.$_SERVER['PHP_SELF'].'?pn='.$First.'">Начало</a> &nbsp; &nbsp; ';
			$paginationCtrls .= '<a href="'.$_SERVER['PHP_SELF'].'?pn='.$previous.'">Предыдущая страница</a> &nbsp; &nbsp; ';
		...
		if ($pagenum != $last) {
			...
			$paginationCtrls .= '<a href="'.$_SERVER['PHP_SELF'].'?pn='.$Last.'">В конец</a>';
		}
	}
	$list = '';
